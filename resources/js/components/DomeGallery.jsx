...
function buildItems(pool, seg) {
  ...
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, src: '', alt: '', videoSrc: null, is_video: false }));
  }

  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, alt: '', videoSrc: null, is_video: false };
    }
    return { 
      src: image.src || '', 
      alt: image.alt || '', 
      videoSrc: image.videoSrc || null, 
  ...
  return coords.map((c, i) => ({
    ...
    videoSrc: usedImages[i].videoSrc,
    is_video: usedImages[i].is_video
  }));
}
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const originalImg = overlay.querySelector('img, video');
      if (originalImg) {
  ...
  const openItemFromElement = useCallback(
    el => {
      ...
      if (isVideo && videoSrc) {
        ...
        // Explicitly trigger play to guarantee autoplay
        setTimeout(() => {
